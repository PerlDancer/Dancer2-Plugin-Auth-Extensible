...
BEGIN {
    ...
    $ENV{DANCER_ENVIRONMENT}     = 'mail-from-plugin-email';
...
like $email->get_header('X-Mailer'), qr{MyTestMailer},
  "... and X-Mailer header is set from Plugin::Email config";

cmp_deeply $delivery->{envelope},
  { from => 'plugin@example.com', to => ['james@example.com'] },
  "... and we see from address as per Plugin::Email config.";
